...
export default function DetailPage() {
  const {movieId3}=useParams(); // useParams 는 react-router-dom 에서 제공하는
...
// useEffect 함수 요청 , 호출 
  ...
  useEffect(()=>{
    async function fetchData(){
      const request=await axios.get(`/movie/${movieId3}`); // baseurl의 값이 있기 때문에 그 이후 내가 연결하고 싶은
